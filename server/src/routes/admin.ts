/**
 * Admin routes:
 *   GET /admin/api — админ-команды (12 шт)
 *
 * Аутентификация: query parameter ?token=ADMIN_PASS (из env).
...
router.get('/api', async (req: Request, res: Response) => {
  try {
    ...
    res.json({ status: 'error', error: err.message || 'Admin API error' });
  }
});
